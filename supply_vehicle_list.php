	...
	<div id="wrapper">
        ...
        <div id="wrapper-inner">
            <div id="firstpane" class="menu_list">
                <h3 class="menu_head current " >共乘類</h3>
            ...
            <div id="content">
				<?php
					$type2_array = array("car","motor","other");
					$type2_cn = array("  /  汽車","  /  機車","  /  其他");
					$tmp = "";
				...
				?>
                <div id="thisH1"><h1 style="color:#476b6b">共乘類<?php echo "<small style='color:#808080'>$tmp</small>";?></div>
                <div id="ThisFilter">
					<div id="div1">
                ...
                <div class="container">
                    <?php
                        $pagesize = 12;
						if($type2 == 'all'){
                        	$sql_total = "select count(*) from supply_list where s_type='vehicle' and s_status = 'on' ";
						}else{
							$sql_total = "select count(*) from supply_list where s_type='vehicle' and s_type2='".$type2."' and s_status = 'on' ";
						}
                    ...
                    <div class="team-members row">
						<?php
                            if(isset($_POST['filter'])){
								$filter = $_POST['filter'];
								if($type2 == 'all'){
									$sql_query = "select a.*, b.*, c.* from supply_vehicle_list a LEFT OUTER JOIN user_list b on a.provider = b.uid LEFT OUTER JOIN supply_list c on a.id = c.s_id  where s_type = 'vehicle' and s_status = 'on' order by ".$_POST['filter']." LIMIT ".$offset.",".$pagesize.""; 
								}else{
									$sql_query = "select a.*, b.*, c.* from supply_vehicle_list a LEFT OUTER JOIN user_list b on a.provider = b.uid LEFT OUTER JOIN supply_list c on a.id = c.s_id  where s_type = 'vehicle' and s_status = 'on' and s_type2 = '".$type2."' order by ".$_POST['filter']." LIMIT ".$offset.",".$pagesize.""; 
								}
							}else if(isset($_GET['filter'])){
								$filter = $_GET['filter'];
								if($type2 == 'all'){
									$sql_query = "select a.*, b.*, c.* from supply_vehicle_list a LEFT OUTER JOIN user_list b on a.provider = b.uid LEFT OUTER JOIN supply_list c on a.id = c.s_id  where s_type = 'vehicle' and s_status = 'on' order by ".$_GET['filter']." LIMIT ".$offset.",".$pagesize."";
								}else{
									$sql_query = "select a.*, b.*, c.* from supply_vehicle_list a LEFT OUTER JOIN user_list b on a.provider = b.uid LEFT OUTER JOIN supply_list c on a.id = c.s_id  where s_type = 'vehicle' and s_status = 'on' and s_type2 = '".$type2."' order by ".$_GET['filter']." LIMIT ".$offset.",".$pagesize."";
								}
							}else{
								$filter = "post_date DESC";
								if($type2 == 'all'){
									$sql_query = "select a.*, b.*, c.* from supply_vehicle_list a LEFT OUTER JOIN user_list b on a.provider = b.uid LEFT OUTER JOIN supply_list c on a.id = c.s_id  where s_type = 'vehicle' and s_status = 'on' order by post_date DESC LIMIT ".$offset.",".$pagesize."";
								}else{
									$sql_query = "select a.*, b.*, c.* from supply_vehicle_list a LEFT OUTER JOIN user_list b on a.provider = b.uid LEFT OUTER JOIN supply_list c on a.id = c.s_id  where s_type = 'vehicle' and s_status = 'on' and s_type2 = '".$type2."' order by post_date DESC LIMIT ".$offset.",".$pagesize."";
								}
							}
						...
						<?php
							if(ceil($total[0]) != 0){
								if ($currentpage > 1) {		// 若果正在顯示第一頁，無需顯示「前一頁」連結
									echo "<li><a href='supply_vehicle_list.php?type2=".$type2."&currentpage=1&filter=".$filter."'> 第一頁 </a></li>";		// 使用 << 連結回到第一頁
									$prevpage = $currentpage - 1;		// 前一頁的頁數
									echo "<li><a href='supply_vehicle_list.php?type2=".$type2."&currentpage=".$prevpage."&filter=".$filter."'>  <  </a></li>";			// 使用 < 連結回到前一頁
								}
								$range = 3; 	// 顯示的頁數範圍
								for ($x = (($currentpage - $range) - 1); $x < (($currentpage + $range) + 1); $x++) {  // 顯示當前分頁鄰近的分頁頁數
									if (($x > 0) && ($x <= $totalpages)) { 		// 如果這是一個正確的頁數...
										...
										echo "<li id='thisSelect'><a href='supply_vehicle_list.php?type2=".$type2."&currentpage=".$x."&filter=".$filter."'>".($x)."</a></li>"; //不使用連結, 但用高亮度顯示
										} else {								// 如果這一頁不是當前頁數...
										echo "<li><a href='supply_vehicle_list.php?type2=".$type2."&currentpage=".$x."&filter=".$filter."'>".($x)." </a></li>";		// 顯示連結
										} 
									} 
								} 
								 if ($currentpage != $totalpages) {	// 如果不是最後一頁, 顯示跳往下一頁及最後一頁的連結
									$nextpage = $currentpage + 1;		// 下一頁的頁數
									echo "<li><a href='supply_vehicle_list.php?type2=".$type2."&currentpage=".$nextpage."&filter=".$filter."'>  >  </a></li>";			// 顯示跳往下一頁的連結
									echo "<li><a href='supply_vehicle_list.php?type2=".$type2."&currentpage=".$totalpages."&filter=".$filter."'> 最後一頁 </a></li>";		// 顯示跳往最後一頁的連結
								}
